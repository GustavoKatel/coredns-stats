...
func (s *StatsBackendSuite) TestBackendPostgres() {
	s.Run("Start runs the init query", func() {
		prefix := faker.Word()
		_, err := stats.PrepareStatsBackend(s.connString, 1, 1*time.Second, prefix, s.logger)
	...
	s.Run("store stats", func() {
		prefix := faker.Word()
		backend, err := stats.PrepareStatsBackend(s.connString, 1, 1*time.Second, prefix, s.logger)
		s.Require().NoError(err)
